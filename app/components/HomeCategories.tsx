...
const CategorySection = ({ category }: { category: Category }) => {
  const {
    ...
    error: productsError,
    isLoading
  } = useFetch<ProductsResponse>(`${process.env.NEXT_PUBLIC_API_BASE_URL}/products/${category.id}`)
    ...
    <div className='mb-10'>
      <div className='flex justify-between items-center mb-4'>
        ...
        <Link
          ...
          className='text-[#EF6B4A] font-semibold hover:underline'
        >
          View All
        </Link>
      </div>
...
const Categories = () => {
  const {
    ...
    isLoading
  } = useFetch<{ category: Category[] }>('https://assign-api.piton.com.tr/api/rest/categories')
